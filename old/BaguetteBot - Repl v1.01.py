#NOTE: This version is designed to run in the Repl.it IDE. Some things have been changed.

DraggieBot_version = "v1"
...
async def bot(ctx):
    ...
    try:
        ...
        print("I'm not sure you have the correct syntax. Use `.bot <CODE> <AMOUNT> <NAME/random>`. The `<NAME>` section can be set to `random`, this will auto generate a Kahoot 'safe' name.\nExample: `.bot 8261910 138 Test player`\n\nNote: Bots do not answer questions, *yet*.")
        return
    ...
    for client in range(botNum):
        ...
        if ogName == 'random':
            ...
            if joins == targetJoins:
                print("Done!")
                if botNum >= 2:
                    print((str ("Done! `")) + (str (joins)) + (str ("` bots have been added to `")) + (str (code)) + (str ("`.")))
                else:
                    print((str ("Done! `")) + (str (joins)) + (str ("` bot has been added to `")) + (str (code)) + (str ("`.")))
...
async def stats(ctx):
    ...
    embed.add_field(name="**Voice Channels:**", value="Disabled")
