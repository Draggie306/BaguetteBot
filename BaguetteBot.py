...
# Attach necessary attributes to client for cogs to access
client.BASE_DIR = BASE_DIR
client.S_SLASH = S_SLASH
client.EMOJI_COINS = EMOJI_COINS
client.EMOJI_NOLWENNIUM = EMOJI_NOLWENNIUM
client.NAME_NOLWENNIUM = NAME_NOLWENNIUM
client.TESTER_GUILD_IDS = TESTER_GUILD_IDS
client.IDK_WHAT_U_MEAN = IDK_WHAT_U_MEAN
client.Roles = Roles
client.GlobalLogDir = GlobalLogDir
client.DRAGGIEBOT_VERSION = DRAGGIEBOT_VERSION
client.BUILD = BUILD
client.YTAPI_STATUS = YTAPI_STATUS
client.AUDIO_SUBSYSTEM = AUDIO_SUBSYSTEM
client.SCAPI_STATUS = SCAPI_STATUS
...
# stats command has been moved to cogs/info.py
...
# coins command has been moved to cogs/economy.py



#   buy command has been moved to cogs/economy.py
...
# CoinsButtons2 class has been moved to cogs/economy.py
...
# Helper functions (get_user_settings, first_save_user_settings, nolwenniumUserDirectory, get_nolwennium, get_coins, update_coins) have been moved to cogs/economy.py



async def bot_runtime_events(event_int):
    client.bot_events = client.bot_events + event_int
...
# Attach helper functions to client for cogs to access
client.slash_log = slash_log
client.error_code = error_code
client.bot_runtime_events = bot_runtime_events
...
async def on_ready():
    ...
    global rolePrivate, hasPrivate, hasAdmin
    client.ready_start_time = time.time()
    ...
    client.rolePrivate = rolePrivate  # Attach to client for cogs
...
async def main_async():
    """Async main function to load cogs before starting the bot"""
    await load_cogs()
    if not BETA_BOT:
